...
export default function App() {
  ...
  const handlePublishToggle = async (nextPublished: boolean) => {
    ...
    try {
      ...
      const publishPageId =
        resolvedPageId && project.pages.some((page) => page.id === resolvedPageId)
          ? resolvedPageId
          : project.pages[0]?.id;
      if (publishPageId) {
        payload.publicPageId = publishPageId;
      }
  ...
  return (
    <ThemeProvider tokens={themeTokens} onTokensChange={setThemeTokens}>
      <div className="min-h-screen bg-gradient-to-br from-black via-slate-950 to-slate-900 text-slate-100">
        <div className="mx-auto flex min-h-screen w-full max-w-none flex-col gap-16 px-6 py-12">

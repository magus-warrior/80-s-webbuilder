...
- Public-facing pages now remove the editor/admin header container entirely, so visitors only see authored page content.
...
- Root node rendering now uses a shared Bootstrap-like container (responsive max-width breakpoints) across editor canvas preview, rendered preview cards, and public pages so desktop proportions stay stable.
- Layout containers are no longer manually resizable in the editor, preventing accidental width shrinkage that made desktop sections look inconsistent.
- Grid/container layout rendering now uses the same responsive auto-fit column behavior in both the builder canvas and published pages, reducing mismatches between edit and live views.
...
- Element move handles now appear only for the currently selected node, keeping the canvas cleaner while preserving direct drag controls.
...
- The editor canvas now scrolls vertically as content grows, so you can keep building sections down the page without hitting a visual cutoff.
- One-click block/primitive insertion now respects container context: if a layout container is selected, new elements are inserted into that container instead of being appended at the canvas root.
